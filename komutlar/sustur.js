...
exports.run = async (client, message, args) => {
  ...
  if (db.has(`mLog_${message.guild.id}`) === false) return message.reply('Mod log ayarlı değil');
  let modlog = message.guild.channels.get(db.fetch(`mLog_${message.guild.id}`).replace("<#", "").replace(">", ""));
  if (db.has(`sRol_${message.guild.id}`) === false) return message.reply('Susturma rolü ayarlı değil');
